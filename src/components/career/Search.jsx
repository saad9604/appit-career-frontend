...
const Search = () => {
  ...
  const handleSearch = () => {
    if (searchValue.trim()) { // Only redirect if there's search text
      ...
      console.log(`Redirecting to career search with query: ${searchValue.trim()} and filters:`, filters);
    }
  };
  ...
  const removeFilter = (type, value) => {
    setFilters(prev => {
      if (type === 'jobType') {
        return { ...prev, jobType: prev.jobType.filter(item => item !== value) };
      } else if (type === 'experienceLevel') {
        return { ...prev, experienceLevel: prev.experienceLevel.filter(item => item !== value) };
      }
      return prev;
    });
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center justify-center max-w-4xl mx-auto gap-6 sm:gap-8 md:gap-10">
        ...
        <h2 
          ref={headingRef}
          className="text-center font-jost text-3xl sm:text-4xl md:text-[40px] lg:text-[48px] font-semibold leading-[120%] w-full transition-all duration-500 ease-out"
        ...
        <p 
          ref={subheadingRef}
          className="text-center font-jost text-lg sm:text-xl md:text-[21px] lg:text-[24px] font-normal leading-[120%] w-full max-w-[800px] transition-all duration-500 ease-out"
          style={{ opacity: 0, transform: 'translateY(20px)' }}
        >
