...
const MoreJobCards = ({ className }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (typeof window !== 'undefined' && window.innerWidth < 340) {
        ...
        cardsRef.current.forEach((card) => {
          if (card) {
            card.classList.add('extra-small-screen');
          }
        });
      } else {
        cardsRef.current.forEach((card) => {
          if (card) {
            card.classList.remove('extra-small-screen');
          }
        });
      }
    };
  ...
  return (
    <section 
      ref={sectionRef}
      className={`w-full py-6 xs:py-8 sm:py-10 md:py-12 lg:py-16 px-2 xs:px-3 sm:px-4 md:px-6 lg:px-8 ${className || 'bg-[#F6F6F6]'}`}
    >
      <div className="max-w-[1240px] mx-auto flex flex-col items-center gap-4 xs:gap-5 sm:gap-6 md:gap-8 lg:gap-10">
        <h2 
          ref={headingRef}
          className="w-full text-center text-base xs:text-lg sm:text-xl md:text-2xl lg:text-[32px] font-semibold leading-[120%] font-jost transition-all duration-500 ease-out"
          style={{ opacity: 0, transform: 'translateY(20px)' }}
        >
          Explore More <span className="text-[#EC1C26]">Current Openings</span>
        </h2>
        
        <div className="w-full grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-3 xs:gap-4 sm:gap-5 md:gap-6 lg:gap-8">
          {jobs.map((job, index) => (
            <div 
              key={index}
            ...
            >
              <div className="w-full flex flex-row justify-between items-start gap-4">
                ...
                <button className="min-w-[90px] sm:min-w-[120px] py-1.5 sm:py-2.5 px-3 sm:px-4 rounded-[24px] bg-[#0066B3] text-white text-xs sm:text-base font-semibold leading-[120%] font-jost flex justify-center items-center hover:bg-[#EC1C26] transition-colors h-fit mt-1"
                  onClick={() => {
